...
def scanline_filling(P: list[QPointF]) -> list[tuple[int, int, int]]:
    ...
    def fill(x_values: list[int], y: int, pixel_spans: list):
        for i in range(0, len(x_values), 2):
            x0 = x_values[i]
            x1 = x_values[i + 1]
            pixel_spans.append((y, x0, x1))
